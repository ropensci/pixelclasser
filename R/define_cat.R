...
#' @param \dots a list of \code{pixel_subcat} objects, or of \code{rule} objects
#'   in case that subcategories are not needed. A mixed list of rule and
...
#'   class \code{rule} or \code{pixel_subcat}, but not a mixture. In this case
...
#' rule03 <- define_rule("R03", "g", "b", c(0.35, 0.29), c(0.49, 0.178), ">=")
#' rule04 <- define_rule("R04", "g", "b", c(0.35, 0.253), c(0.45, 0.253), "<")
#'
...
#'
#' # A single category defined by a set of rules, not subclasses
...
define_cat <- function(cat_name, cat_colour, ...){
  ...
  object_list <- list(...)
  object_names <- sapply(match.call(expand.dots = FALSE)$..., deparse)
  ...
  for (i in seq_along(object_list)){
    if (identical(class(object_list[[i]]), "rule")){
      ...
      if (identical(class(object_list[[i]]), "pixel_subcat")){
        ...
        stop("Object ", object_names[i],
             " is neither of class rule nor of class pixel_subcat.\n",
             call. = FALSE)
      }
    }
  }
  ...
  if (number_of_rules > 0){
    ...
    result <- c(list(name = cat_name,
                     colour = grDevices::col2rgb(cat_colour)/255),
                subcats = vector(mode = "list", length = length(object_list)))
    for (i in seq_along(object_list)){
